...
// Copyright (c) 2019 The Stronghands developers
...
// the following overflows with large coinsupply
//    return ((nValue*1000)/(3*((int)GetSerializeSize(SER_DISK,0)+148)) < CTransaction::nMinRelayTxFee);
    return (nValue/(3*((int)GetSerializeSize(SER_DISK,0)+148)) < (CTransaction::nMinRelayTxFee/1000));
}
...
{
//    CBigNum bnSubsidy = MINT_PROOF_OF_WORK;
//    int64 nSubsidy = bnSubsidy.getuint64();
    int64 nSubsidy = MINT_PROOF_OF_WORK;

    if (nBestHeight < 10)
	{
		nSubsidy = 1650000000 * COIN;
	}
    else if (nBestHeight == 11)
	{
		nSubsidy = 1000000 * COIN; // coins to stake during swap
	}
    else if (nBestHeight > 11 && nBestHeight < 5000) // 9 days
	{
		nSubsidy = 1 * COIN;
	}
...
{
    int64 nSubsidy = MINT_PROOF_OF_STAKE;

    if (nBestHeight < 5000) // 9 days
	{
		nSubsidy = 1 * COIN;
	}
...
static const int64 nTargetTimespan = 36 * 60 * 60;  // 36 hours
...
void ThreadScriptCheck() {
    RenameThread("stronghands-scriptch");
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        /* Genesis Block:
14 august
block.GetHash() == 00000000a2ec5ccae9a06ba506ce197aadfe5efa828d87790fb05e6b70818ada
block.hashMerkleRoot == 8122d947b687bac4150eb2a836ff5bd8d7dfde69c2d244e4081ef65c2c5aa045
block.nTime = 1565784000 
block.nNonce = 457553359

        assert(block.hashMerkleRoot == uint256("0x8122d947b687bac4150eb2a836ff5bd8d7dfde69c2d244e4081ef65c2c5aa045"));
static const uint256 hashGenesisBlockOfficial("0x00000000a2ec5ccae9a06ba506ce197aadfe5efa828d87790fb05e6b70818ada");
*/

        // Genesis block
        const char* pszTimestamp = "Look at how a single candle can both defy and define the darkness";
        CTransaction txNew;
        txNew.nTime = 1565784000;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
//            printf("Minter thread sleeps while sync at %f\n",Checkpoints::GuessVerificationProgress(pindexBest));
            printf("Minter thread sleeps while downloading blocks.\n");
